...
void getNegValid() {
    ...
    for (INT i = 0; i < validTotal; i++) {
        //printf("in getNegValid, i is %ld\n", i);
        //printf("check negValidList is %ld, validList is %ld\n", (INT) negValidList, (INT) validList);
        //printf("%ld %ld %ld\n", validList[i].h, validList[i].r, negValidList[i].t);
...
void getValidBatch(INT *ph, INT *pt, INT *pr, INT *nh, INT *nt, INT *nr) {
    //printf("validTotal is %ld\n", validTotal);
    getNegValid();
    for (INT i = 0; i < validTotal; i++) {
        //printf("in getValidBatch, i is %ld\n", i);
        ph[i] = validList[i].h;
        pt[i] = validList[i].t;
        pr[i] = validList[i].r;
...
REAL aveAcc;
extern "C"
double test_triple_classification(REAL *relThresh, REAL *score_pos, REAL *score_neg) {
