...
class Config(object):
    ...
    '''
    def __init__(self):
        base_file = os.path.abspath(os.path.join(os.path.dirname(__file__), '../origin_base/Base.so'))
        ...
        self.lib.test_triple_classification.restype = ctypes.c_double
        ...
        with self.graph.as_default():
            ...
            with self.sess.as_default():
                ...
                with tf.variable_scope("model", reuse=None, initializer = initializer):
                    if kg == 'AIO' or 'subgeonamesA':
                        device = '/device:GPU:0'
                    elif kg == 'geonames' or kg == 'worldlift' or kg == 'whisky' or kg == 'tharawat' or kg == 'lex':
                        device = '/device:GPU:0'
                    else:
                        device = '/device:GPU:1'
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                if self.test_triple_classification:
                    ...
                    return self.lib.test_triple_classification(self.relThresh_addr, res_pos.__array_interface__['data'][0], res_neg.__array_interface__['data'][0])
