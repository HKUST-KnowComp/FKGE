...
void importTypeFiles() {
	...
	/*for(INT k = 0; k < 100000000; k++) {
		printf("flag b is: relationTotal %ld", relationTotal);
	}*/
	for (INT i = 0; i < relationTotal; i++) {
		INT rel, tot;
		tmp = fscanf(f_type, "%ld%ld", &rel, &tot);
		/*for(INT k = 0; k < 100000000; k++) {
			printf("flag a is: relationTotal %ld, rel %ld, tot %ld", relationTotal, rel, tot);
		}*/
